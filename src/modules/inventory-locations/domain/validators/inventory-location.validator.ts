...
export class InventoryLocationValidator {
  ...
  async validateCode(code: string): Promise<void> {
    ...
    const existingLocation = await this.inventoryLocationRepository.findByCode(code);
    if (existingLocation) {
      throw new DomainException(
  ...
  async validateType(type: string): Promise<void> {
    ...
    const validTypes = ['Warehouse', 'Store'];
    if (!validTypes.includes(type)) {
      throw new DomainException(
        'Invalid location type. Must be either Warehouse or Store',
        HttpStatus.BAD_REQUEST
      );
    }
  }
