...
export class PriceCategoryRepository extends BaseRepository<PriceCategory> {
  ...
  async findByTypeAndName(type: string, name: string): Promise<PriceCategory | null> {
  ...
  async findAllGroupedByType(search?: string): Promise<Record<string, PriceCategory[]>> {
    const query = this.priceCategoryRepository.createQueryBuilder('price_category')
      .where('price_category.deleted_at IS NULL');

    if (search) {
      query.andWhere('LOWER(price_category.name) LIKE LOWER(:search)', {
        search: `%${search}%`
      });
    }
