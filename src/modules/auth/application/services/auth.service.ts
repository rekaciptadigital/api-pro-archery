...
export class AuthService {
  ...
  async register(registerDto: RegisterDto): Promise<AuthResponse> {
    ...
    return this.authResponseHelper.createLoginResponse(
      user,
      tokens,
      HttpStatus.CREATED
    );
  ...
  async refreshToken(refreshToken: string): Promise<AuthResponse> {
    const { user, tokens } =
      await this.authenticationService.refreshUserToken(refreshToken);
    return this.authResponseHelper.createLoginResponse(user, tokens);
  }
  ...
  private async createRefreshToken(
    ...
    const refreshTokenExpiration =
      this.tokenConfigService.getRefreshTokenExpiration();
